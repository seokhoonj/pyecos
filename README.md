...
주요국(미·일·중·유로·영·한·캐나다·인도 등) 정책금리와 시장금리까지 다룹니다.

자주 쓰는 지표는 `ecos.rate.base` 같은 **접근자**로 바로 꺼내고, 그 밖의 통계는
통계표 코드로 조회합니다.
...
이 패키지는 ECOS API 키가 필요합니다. <https://ecos.bok.or.kr/api/>에서 무료로 발급받으세요.
...
통계표 코드를 외우지 않고, 자주 쓰는 지표를 `ecos.그룹.지표` 형태로 바로 꺼냅니다.
...
└── world/
    └── rate/
        ...
        │   ├── ca                        # 캐나다 정책금리
        │   └── india                     # 인도 정책금리
        └── market/                       # 유로 지역은 시장금리 단일계열이 ECOS에 없어 독일(de)로 대용
            ├── us/
            │   ├── long                  # 미국 장기금리
            │   └── short                 # 미국 단기금리
            ├── jp/
...
| world | `ecos.world.rate.market.ca.long` | 캐나다 장기금리 |
| world | `ecos.world.rate.market.ca.short` | 캐나다 단기금리 |
| world | `ecos.world.rate.market.cn.long` | 중국 장기금리 |
| world | `ecos.world.rate.market.cn.short` | 중국 단기금리 |
| world | `ecos.world.rate.market.de.long` | 독일 장기금리 |
| world | `ecos.world.rate.market.de.short` | 독일 단기금리 |
| world | `ecos.world.rate.market.india.long` | 인도 장기금리 |
| world | `ecos.world.rate.market.india.short` | 인도 단기금리 |
...
위 목록에 없는 통계는 통계표 코드로 직접 조회합니다. ECOS가 제공하는 6가지 조회를 그대로
...
위 목록에 없는 통계표의 코드를 찾을 때 — API 호출 없이 패키지에 내장된 통계표 목록에서 바로
검색합니다.
