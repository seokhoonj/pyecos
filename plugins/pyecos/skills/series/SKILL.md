...
vendor error comes back from the CLI as a one-line `pyecos: <message>` -- relay it as-is
...
That puts the `pyecos` command on PATH. The key can also be stored in
`~/.config/pyecos/credentials.json` as `{"ECOS_API_KEY": "..."}`. Without a key the
CLI exits with `pyecos: no ECOS API key ...`; relay that and point the user at the
ECOS site.

## Running

```
pyecos series "<STAT_CODE>" [--item CODE] [--cycle ...] [--start ...] [--end ...] [options]
```

Options (`pyecos series --help` is the source of truth):
...
3. **Relay the result.** Show the CLI's stdout. You may trim a long series, but keep the
   summary line.
4. **Error handling.** When the CLI exits non-zero, relay the one-line `pyecos: <message>`
   from stderr as-is. Common ones:
   - `command not found: pyecos` -> not installed; point the user at `pipx install pyecos`.
